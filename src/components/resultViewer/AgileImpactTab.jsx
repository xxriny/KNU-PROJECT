...
import {
  ...
  Package, Globe, Database, ArrowRight,
} from "lucide-react";
...
export default function AgileImpactTab() {
  ...
  return (
    <div className={`h-full flex flex-col p-6 space-y-6 ${isDarkMode ? "text-slate-300" : "text-slate-800"}`}>
      ...
      {/* Results */}
      {result && (
        <div className="space-y-5 animate-fade-in overflow-y-auto flex-1 custom-scrollbar pr-1">
          {/* Summary + Risk */}
      ...
      {!result && !loading && !error && (
        <div className={`flex-1 flex flex-col items-center justify-center gap-4 opacity-30 border-2 border-dashed rounded-3xl ${isDarkMode ? "border-white/10" : "border-slate-200"}`}>
          <GitBranch size={64} />
          <p className="text-lg font-bold">변경 사항을 입력하고 분석을 실행하세요</p>
        </div>
      )}
    </div>
  );
