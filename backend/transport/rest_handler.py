...
async def extract_final_idea_endpoint(req: ExtractFinalIdeaRequest):
    """🚀 Sync 빌드 전 Pre-flight 단계.
    날것의 대화/메모를 정제해 최종 확정 요구사항 마크다운만 추출한다.
    무거운 분석 파이프라인의 입력값을 깨끗하게 만드는 용도이며,
    LLM 1회만 호출하는 가벼운 동기 엔드포인트.
    """
    try:
...
async def get_memos_endpoint(session_id: Optional[str] = None):
    ...
    # 세션 ID가 없으면 빈 결과 반환 (전체 DB leak 방지)
    if not session_id:
        ...
        memos = [
            {
                ...
                "metadata": {
                    ...
                    "reflected_version": m.reflected_version,
                },
            }
            for m in items
        ]
