...
def sa_phase6_node(state: PipelineState) -> dict:
    ...
    if action_type == "CREATE":
        return {
            ...
            "thinking_log": sget("thinking_log", []) + [{"node": "sa_phase6", "thinking": "CREATE 모드 감지. 보안 설계 스킵(선택적)."}],
    ...
    if not mapped_reqs:
        return {
            ...
            "thinking_log": sget("thinking_log", []) + [{"node": "sa_phase6", "thinking": "매핑된 요구사항이 없어 보안 설계 생략"}],
            "current_step": "sa_phase6_done",
        }
    ...
    try:
        ...
        thinking_msg = f"보안 설계 완료: 역할 {len(result.defined_roles)}개, 신뢰 경계 {len(result.trust_boundaries)}개 도출."
        ...
        thinking_msg = f"LLM 호출 실패로 Fallback 적용: {e}"
