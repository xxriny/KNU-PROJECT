...
def test_backend_codegen_merges_node_manifest_with_llm_imports(monkeypatch, tmp_path: Path) -> None:
    target_repo = _init_git_repo(tmp_path / "target")
    ...
    generated_dir = target_repo / "backend" / "generated" / "navigator_dev" / "typescript_express"
    package = json.loads((generated_dir / "package.json").read_text(encoding="utf-8"))

    assert result["status"] == "generated"
    assert package["dependencies"]["cors"] == "^2.8.5"
    assert package["dependencies"]["dotenv"] == "^16.4.5"
    assert package["dependencies"]["@supabase/supabase-js"] == "^2.45.4"
    assert package["devDependencies"]["@types/cors"] == "^2.8.17"
    assert package["devDependencies"]["@types/node"] == "^20.14.10"
    assert "@types/supabase__supabase-js" not in package["devDependencies"]
...
def test_backend_codegen_verifier_skips_node_target_without_dependencies(tmp_path: Path) -> None:
    ...
    assert result["failure_type"] == "dependency_install_required"
...
def test_backend_codegen_verifier_classifies_missing_dependency_failure(tmp_path: Path, monkeypatch) -> None:
    import pipeline.domain.dev.nodes.backend_codegen_verifier as verifier
    ...
    (output_dir / "package.json").write_text(
        '{"scripts":{"test":"vitest run"},"devDependencies":{"vitest":"^1.6.0","typescript":"^5.4.0"}}',
        encoding="utf-8",
    )
    (output_dir / "tsconfig.json").write_text("{}", encoding="utf-8")

    def fake_run_check(name: str, command: list[str], cwd: Path, timeout: int = 120):
        if name == "typecheck":
            return verifier.BackendCodegenVerificationCheck(
                name=name,
                status="failed",
                command=command,
                stderr_tail="src/app.ts(2,18): error TS2307: Cannot find module 'cors'.",
            )
        return verifier.BackendCodegenVerificationCheck(name=name, status="passed", command=command)
    ...
    assert result["checks"][0]["failure_type"] == "missing_dependency"


def test_backend_codegen_verifier_classifies_test_framework_mismatch(tmp_path: Path, monkeypatch) -> None:
    import pipeline.domain.dev.nodes.backend_codegen_verifier as verifier
    ...
    def fake_run_check(name: str, command: list[str], cwd: Path, timeout: int = 120):
        if name == "typecheck":
            return verifier.BackendCodegenVerificationCheck(
                ...
                status="failed",
                command=command,
                stderr_tail="src/__tests__/auth.test.ts(6,1): error TS2304: Cannot find name 'jest'.",
            )
        return verifier.BackendCodegenVerificationCheck(name=name, status="passed", command=command)
    ...
    assert result["status"] == "failed"
    assert result["failure_type"] == "test_framework_mismatch"
...
def test_backend_codegen_verifier_sanitizes_manifest_before_dependency_install(tmp_path: Path, monkeypatch) -> None:
    import pipeline.domain.dev.nodes.backend_codegen_verifier as verifier
    ...
    (output_dir / "package.json").write_text(
        json.dumps({
            "dependencies": {"@supabase/supabase-js": "^2.43.4"},
            "devDependencies": {"@types/supabase__supabase-js": "^2.43.4"},
        }),
        encoding="utf-8",
    )
    ...
    assert result["dependency_install_result"]["status"] == "passed"
    assert "@types/supabase__supabase-js" not in package["devDependencies"]
    assert any("manifest_sanitization" in action for action in result["next_actions"])
...
def test_backend_codegen_repair_sanitizes_invalid_supabase_types_package(monkeypatch, tmp_path: Path) -> None:
    output_dir = tmp_path / "generated"
    ...
    def fake_call_structured(**kwargs):
        parsed = BackendCodegenRepairOutput(
            thinking="의존성정리",
            summary="Remove invalid Supabase type package.",
            files=[
                GeneratedCodeFile(
                    path="package.json",
                    content=json.dumps({
                        "dependencies": {"@supabase/supabase-js": "^2.43.4"},
                        "devDependencies": {
                            "@types/supabase__supabase-js": "^2.43.4",
                            "typescript": "^5.4.5",
                        },
                    }),
                )
    ...
    result = develop_backend_codegen_repair_node({
        ...
        "backend_codegen_verification": {
            ...
            "checks": [{"name": "dependency_install", "status": "failed", "stderr_tail": "E404 @types/supabase__supabase-js"}],
        },
    })["backend_codegen_repair_result"]
    ...
    assert result["status"] == "repaired"
    assert result["manifest_sanitization"][0]["removed"][0]["package"] == "@types/supabase__supabase-js"
    assert "@types/supabase__supabase-js" not in package["devDependencies"]
    assert package["dependencies"]["@supabase/supabase-js"] == "^2.43.4"


def test_backend_codegen_repair_context_includes_manifest_imports_and_failure_type(tmp_path: Path) -> None:
    import pipeline.domain.dev.nodes.backend_codegen_verifier as verifier

    output_dir = tmp_path / "generated"
    (output_dir / "src").mkdir(parents=True)
    (output_dir / "package.json").write_text(
        '{"dependencies":{"express":"^4.18.3"},"devDependencies":{"vitest":"^1.6.0"}}',
        encoding="utf-8",
    )
    (output_dir / "src" / "app.ts").write_text(
        "import cors from 'cors';\nimport express from 'express';\n",
        encoding="utf-8",
    )

    payload = json.loads(verifier._repair_user_message(
        codegen={"language": "typescript", "framework": "express", "test_command": "npm test"},
        verification={
            "status": "failed",
            ...
            "checks": [{
                "name": "typecheck",
                "status": "failed",
                "stderr_tail": "src/app.ts(1,18): error TS2307: Cannot find module 'cors'.",
            }],
        },
        output_dir=output_dir,
    ))

    assert payload["failure_type"] == "missing_dependency"
    assert payload["package_manifest"]["path"] == "package.json"
    assert {"path": "src/app.ts", "source": "cors", "package": "cors"} in payload["node_import_inventory"]
