"""
main_agent, domain_agent, QA gate, codegen, verifier, repair agent, integration QA, branch/PR 단계가 주고받는 데이터 구조를 정의
각 노드는 이 모델을 기준으로 structured output을 생성하거나 state payload를 검증
"""
...
#각 도메인 agent가 실행해야하는 표준 payload
class DevTaskInfo(BaseModel):
    ...
    run_id: str = ""
    attempt: int = 1
    source: str = "develop_main_agent"


class DevTaskContext(BaseModel):
    approved_stacks: List[str] = Field(default_factory=list)
    approved_stack: dict = Field(default_factory=dict)
    sa_bundle: dict = Field(default_factory=dict)
    target_api_specs: List[dict] = Field(default_factory=list)
    target_table_specs: List[dict] = Field(default_factory=list)
    target_components: List[str] = Field(default_factory=list)
    ...
    integration_feedback: dict = Field(default_factory=dict)
    rework_instruction: dict = Field(default_factory=dict)


class DevTaskConstraints(BaseModel):
    no_dummy_code: bool = True
    no_placeholder_business_logic: bool = True
    no_mock_business_logic: bool = True
    no_unapproved_stack: bool = True
    no_extra_api: bool = True
    no_missing_sa_api: bool = True
    preserve_request_response_fields: bool = True
    preserve_pm_requirement_trace: bool = True


class DevTask(BaseModel):
    task_info: DevTaskInfo
    context: DevTaskContext = Field(default_factory=DevTaskContext)
    instruction: str = ""
    acceptance_criteria: List[str] = Field(default_factory=list)
    constraints: DevTaskConstraints = Field(default_factory=DevTaskConstraints)
