...
def test_hybrid_rag_integration():
    ...
    # PROJECT_RAG에 이미 게시판 조회 기능이 적재된 상태(메타만 표현)
    existing_rag_meta = {
        "frameworks": ["FastAPI", "MariaDB"],
        "entities": ["Post"],
        "apis": ["GET /api/v1/posts (목록)", "GET /api/v1/posts/{id} (상세)"],
    }
    ...
    state = {
        ...
        "rag_index_status": {
            "has_index": True,
            "chunk_count": 24,
            "session_id": "st_01_hybrid_session",
        },
    ...
    judge_system(scenario_name, requirements, existing_rag_meta, final_output)
