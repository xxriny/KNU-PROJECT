...
def sa_phase8_node(state: PipelineState) -> dict:
    ...
    # 최종/SA 요약 (sa_reverse_context 제거 대체)
    mode = ((sget("metadata", {}) or {}).get("action_type") or sget("action_type", "") or "").strip() or "unknown"
    feasibility_status = (sget("sa_phase3", {}) or {}).get("status") or "unknown"
    summary = (
        f"[{mode}] 요구사항 {len(rtm)}개를 기준으로 SA 분석을 완료했습니다. "
        f"타당성 상태는 {feasibility_status}이며, "
        f"병렬 개발 배치는 {len(parallel_batches)}개입니다."
    )
